#! /usr/bin/python
# coding:utf-8
# https://blog.csdn.net/mzbqhbc12/article/details/54992954?utm_source=blogxgwz1
    ...
    def matchs(self):
       ...
       # color_data =data['valItemInfo'] ['skuList']  ### 这里获得商品的颜色信息列表   包括颜色编码  颜色名称,商品skuID
        if data:
            ...
            self.newdata={"DETAIL":detail_image,"id":self.id,}
        else:
            self.newdata = {}
...
#                threading.Thread(target=self.download_color,args=(x,)).start()
            for x in range(len(self.newdata['DETAIL'])):
                index = str(self.id) + str(x) 
                threading.Thread(target=self.download_detail,args=(self.newdata['DETAIL'][x],index)).start()
        return
...
if __name__ =="__main__":
    tb_db = MongDB()
    taobao = TMSpider("Data/Tmall")

    detail_sid = tb_db.get_from_mongo()
    for i in range(len(detail_sid)):
        taobao.set_sid(detail_sid[i]['data_sid'])
        taobao.matchs()
        taobao.download()
